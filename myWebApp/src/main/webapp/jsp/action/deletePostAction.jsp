...
<%--1. 로그인한 사용자가 본인의 글을 삭제하려는 경우 : sessionID == bPassword == userID--%>
<%--2. 로그인하지 않은 사용자가 본인의 글을 삭제하려는 경우 : bPassword 로 확인--%>
<%--3. 로그인한 사용자가 Guest의 글을 삭제하려는 경우--%>
<%
    String userID = null;
...
<script>
    ...
    if (userInput === "<%=board.getbPassword()%>") {
        if ("<%=board.getUserID()%>" !== "Guest") {
            ...
            location.href = 'deletePostAction.jsp?ok=true&bID=<%=bID%>';
        }
    } else {
...
<%
    ...
    if (ok.equals("true") && userID.equals(board.getUserID())) {
        ...
        if(result!= -1){
            ...
            writer.println("alert('게시글이 삭제되었습니다.')");
            writer.println("location.href = '../view/board.jsp'");
            writer.println("</script>");
        }else{
            PrintWriter writer = response.getWriter();
            writer.println("<script>");
            writer.println("alert('게시글 삭제에 실패하였습니다.')");
...
<div class="container-fluid" style="margin-top: 20px; width: 70%">
    ...
    <form method="POST" action="../action/updatePostAction2.jsp?bID=<%=bID%>">
        ...
        <div class="form-inline">
            ...
            %>
            &lt;%&ndash;로그인하지 않은 사용자에게만 보여주기&ndash;%&gt;
